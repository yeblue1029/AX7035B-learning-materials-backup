...
existing GitHub Pages artifact):
    ...
    viewer/ai/
      ...
      index.json          machine routing index for agents/scripts
      AI_USAGE.txt        plain-text instructions for AIs
      docs/<doc_id>/
        index.html        doc landing page — real <a href> to every artifact
        ...
        full.html         lightweight HTML version (per-page [Page TXT]/[Blocks])
        pages/index.html  static index linking every NNNN.txt (no JavaScript)
        pages/0001.txt    one file per physical PDF page (1-based)
        blocks/index.html static index linking every NNNN.json (no JavaScript)
        blocks/0001.json  per-page text blocks with bboxes (embedded or OCR)

Hyperlink-driven navigation (web chat AI safe-to-open compatibility):
  Web chat AIs (ChatGPT/Gemini/...) can only open URLs that already appeared in
  a previous page/message, so navigation must be a chain of real <a href>
  links:  /ai/  ->  docs/<doc_id>/index.html  ->  full.txt / pages/index.html
  ->  pages/NNNN.txt.  The nav layer (landing pages, page/block indexes,
  full.html per-page links) is generated by `combine` from cached artifacts
  only — it never re-opens a PDF and never re-OCRs.
...
# Navigation layer version (landing pages, pages//blocks/ indexes, full.html
# per-page links). Independent of EXTRACTOR_VERSION / OCR_CONFIG_VERSION on
# purpose: regenerating the nav layer must NEVER invalidate the extraction
# cache (no re-OCR). It is generated by `combine` from cached artifacts.
NAV_VERSION = int(os.environ.get("AI_NAV_VERSION", "1"))
...
def urls_for(rel_path, doc_id):
    ...
    return {
        "ai_landing_url": f"{PAGES_BASE_URL}/ai/docs/{doc_id}/index.html",
...
<p class="lead">本页为静态 HTML，全部文档直接列在下方源码中，无需 JavaScript。
<b>Web Chat AI 请沿本页真实 <code>&lt;a href&gt;</code> 链接导航</b>：点击文档标题进入文档页
（landing），再点击 full.txt / pages / blocks——不需要根据 JSON 字符串自行拼接或打开新 URL。
机器 JSON 接口 <code>index.json</code> 供支持任意 fetch 的 Agent / 脚本使用 ·
说明 <code>AI_USAGE.txt</code> · 人读请用<a href="../">PDF.js 文档中心</a>。
OCR 页文本仅供搜索/定位，关键事实请回原 PDF 复核。</p>
"""
...
def cmd_combine(args):
    ...
    # Hyperlink navigation layer (landing pages, pages//blocks/ indexes,
    # full.html per-page links). Generated from cached artifacts only —
    # no PDF is opened, no OCR is re-run. Must run BEFORE _dir_size so the
    # size guard accounts for the nav pages.
    nav = _generate_nav(docs_dir, manifests)
    ...
    index = {
        ...
        "html_entry": f"{PAGES_BASE_URL}/ai/",
    ...
    print(f"[combine] nav v{NAV_VERSION}: landings {nav['landings']} / "
          f"pages-index {nav['pages_index']} / blocks-index {nav['blocks_index']} / "
          f"full.html patched {nav['full_html_patched']}")
...
# --------------------------------------------------------------------------
# Hyperlink navigation layer (combine step; stdlib only; never re-extracts)
#
# Web chat AIs are typically restricted to opening URLs that already appeared
# in a previous page (safe-to-open). The nav layer therefore chains REAL
# <a href> links:  /ai/ -> docs/<id>/index.html -> artifacts -> pages/NNNN.txt
# --------------------------------------------------------------------------

NAV_PAGE_CSS = ("<style>body{font-family:'Noto Sans CJK SC','WenQuanYi Micro Hei',"
                "sans-serif;max-width:860px;margin:0 auto;padding:24px;line-height:1.6;"
                "color:#1c2330;background:#fff}h1{font-size:20px;word-break:break-all}"
                "small,.meta{color:#57606a}ol{padding-left:20px}li{margin:3px 0}"
                "a{color:#0969da;text-decoration:none}a:hover{text-decoration:underline}"
                ".crumb{font-size:13px;margin-bottom:12px}.badge{font-size:11px;"
                "padding:2px 8px;border-radius:99px;background:#58a6ff;color:#fff}"
                ".badge.lfs_not_materialized,.badge.partial{background:#d29922}"
                ".badge.error,.badge.invalid_pdf,.badge.ocr_failed{background:#f85149}"
                ".badge.text_sparse{background:#8b98a5}ul.links{list-style:none;"
                "padding-left:0}ul.links li{margin:6px 0}</style>\n")

PAGE_FILE_RE = re.compile(r"^(\d{4})\.(txt|json)$")


def _nav_doc_html(title, crumb_href, crumb_label, body_html):
    """Static shell for nav pages. No <script> anywhere (no-JS rule)."""
    return ("<!doctype html>\n<html lang=\"zh-CN\">\n<head><meta charset=\"utf-8\">\n"
            "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">\n"
            f"<title>{html_mod.escape(title)}</title>\n" + NAV_PAGE_CSS +
            "</head>\n<body>\n"
            f"<p class=\"crumb\"><a href=\"{crumb_href}\">&#8592; {html_mod.escape(crumb_label)}</a></p>\n"
            + body_html + "\n</body>\n</html>\n")


def _list_numbered(dir_path, ext):
    """Sorted NNNN.<ext> files present in dir_path."""
    if not os.path.isdir(dir_path):
        return []
    return sorted(f for f in os.listdir(dir_path) if PAGE_FILE_RE.match(f)
                  and f.endswith("." + ext))
...
def _write_numbered_index(dir_path, out_name, m, ext, label):
    ...
    items = "\n".join(
        ...
        for f in files)
    body = (f"<h1>{html_mod.escape(m.get('title', m['filename']))} — {label}</h1>\n"
...
def _write_doc_landing(doc_dir, m, has_text):
    ...
    links.append(f'<li>📥 <a href="{u["original_raw_url"]}">原 PDF — raw 二进制（Agent / 脚本 / 证据复核用）</a></li>')
    links.append(f'<li>👁 <a href="{u["viewer_url"]}">PDF.js 在线阅读（人类读者）</a></li>')
    ...
    body = (f"<h1>{html_mod.escape(m.get('title', m['filename']))}</h1>\n"
            f"<p class=\"meta\">SOURCE_PATH: {html_mod.escape(m['source_path'])} · "
            f"doc_id: {m['doc_id']} · PDF 页数: {m.get('pdf_page_count', 0)} · "
            f"<span class=\"badge {st}\">{st}</span></p>\n"
            f"<p class=\"meta\">页统计: {counts} · SOURCE_SHA256: {m.get('source_sha256', '')}</p>\n"
            + notice +
            '<ul class="links">\n' + "\n".join(links) + '\n</ul>\n'
            '<p class="meta">OCR/mixed 页文本仅供搜索与定位；Pin、IOSTANDARD、寄存器地址、'
            'bit、频率、公式等关键事实必须回原 PDF 复核。</p>\n')
    with open(os.path.join(doc_dir, "index.html"), "w", encoding="utf-8") as fh:
        fh.write(_nav_doc_html(f"{m.get('title', m['filename'])} · AI Reading Path",
                               "../../index.html", "AI Reading Path 文档列表", body))


def _patch_full_html(doc_dir):
    """Insert real per-page links into full.html (idempotent, no re-extraction).

    Cached full.html files were generated before the nav layer existed; this
    patches them in place so every PDF_PAGE heading carries
    [Page TXT] -> pages/NNNN.txt and [Blocks JSON] -> blocks/NNNN.json links.
    """
    path = os.path.join(doc_dir, "full.html")
    ...
    try:
        ...
        return False
    if "class=\"pgnav\"" in h:
        return False  # already patched (fresh builds may include nav natively)
    h, n = re.subn(
        r"<h3>(PDF_PAGE (\d{4})) ",
        r'<h3>\1 <span class="pgnav" style="font-size:12px">'
        r'<a href="pages/\2.txt">[Page TXT]</a> '
        r'<a href="blocks/\2.json">[Blocks JSON]</a></span> ',
        h)
    if n == 0:
        return False
    h = h.replace("<body>\n",
                  '<body>\n<p style="font-size:13px"><a href="index.html">'
                  "&#8592; 文档首页（AI Reading Path landing）</a></p>\n", 1)
...
def _write_index_html(out_dir, index):
    ...
    for d in index["documents"]:
        ...
        landing = f'docs/{d["doc_id"]}/index.html'
        # The document TITLE links to the doc landing page: web chat AIs
        # navigate by clicking real <a href> links (safe-to-open compatible).
        links = [f'<a href="{landing}">doc</a>']
        if d.get("ai_full_text_url"):
            ...
            links.append(f'<a href="docs/{d["doc_id"]}/pages/index.html">pages</a>')
        links.append(f'<a href="docs/{d["doc_id"]}/manifest.json">manifest</a>')
        if d.get("original_raw_url"):
            links.append(f'<a href="{d["original_github_url"]}">GitHub</a>')
        ...
        rows.append(
            ...
            f'<td class="t"><a href="{landing}">{html_mod.escape(d["title"])}</a><br>'
    ...
    doc = (INDEX_HTML_HEAD
           ...
           + '<div class="pill">📄 <a href="AI_USAGE.txt">AI_USAGE.txt</a>（AI 使用说明）</div>'
           + '<div class="pill">📦 <a href="index.json">index.json</a>（机器 JSON 接口：Agent / 脚本）</div>'
...
def _write_ai_usage(out_dir, index):
    ...
    lines = [
        ...
        "or executing PDF.js.",
        "",
        "WEB CHAT AI NAVIGATION (hyperlink-driven — use this first):",
        "  Many web chat AIs can only open URLs that already appeared in a previous",
        "  page or the user's message (safe-to-open policy). Navigate by following",
        "  REAL HTML links — never construct URLs yourself from JSON strings:",
        "",
        f"  1. Open the static list page : {PAGES_BASE_URL}/ai/",
        "     (every document is listed in the raw HTML; no JavaScript needed)",
        "  2. Click a document TITLE -> doc landing page docs/<doc_id>/index.html",
        "  3. From the landing page click: full.txt / full.html /",
        "     pages/index.html / blocks/index.html / manifest.json",
        "  4. From pages/index.html click any page: NNNN.txt (physical PDF page)",
        "",
        "MACHINE JSON INTERFACE (second path — agents/scripts that may fetch any URL):",
