...
const REPO_NAME = process.env.REPO_NAME || "AX7035B-learning-materials-backup";
...
const EXCLUDE_DIRS = new Set([
  ...
  ".idea",
  ".vs",
  ".trae",      // TRAE skill/index tooling, not learning content
  "AI_REPO_INDEX", // generated AI navigation, not source PDFs
]);
...
// Build-output directory name suffixes (from .gitignore) — never hold source PDFs.
const BUILD_OUTPUT_SUFFIXES = [
  ".runs",
  ".cache",
  ".hw",
  ".sim",
...
function isExcludedDir(relDir) {
  ...
  if (posixDir.startsWith("viewer/build") || posixDir.startsWith("viewer/web"))
    return true;
  // build output suffix dirs (e.g. .../project.sim, .../project.runs)
  if (parts.some((seg) => BUILD_OUTPUT_SUFFIXES.includes(seg))) return true;
  return false;
}
...
function walk(root) {
  const out = [];
  if (typeof readdirSync(root, { recursive: true, withFileTypes: true }).catch !== "function") {
    // Node 20+ supports recursive; readdirSync is synchronous (no .catch).
  }
  let entries;
  try {
