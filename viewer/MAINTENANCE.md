...
- **PDF 原文件只存一份**：仓库 4.5 GB，远超 GitHub Pages 1 GB 推荐上限，因此 PDF **不**复制进 Pages。
...
```
// [AX7035B-PDF-LIBRARY PATCH BEGIN] allow cross-origin PDF loading from GitHub raw (CORS-enabled).
...
- 递归识别 `.pdf` 与 `.PDF`，排除 `.git`、`node_modules`、`viewer/build`、`viewer/web`、
  Vivado 构建输出目录（`.runs`/`.cache`/`.hw`/`.sim`/`.gen`/`ip_user_files`）等。
...
1. **Git-LFS 文件（2 个，各 ~130 MB）**：
   ...
   其 `raw` 返回的是 LFS **指针文本**（`version https://git-lfs.github.com/spec/v1 …`），**不是 PDF**。
   因此这两个文件：在线阅读会失败（viewer 拿到的是指针）；AI `curl` 也只能拿到指针。
   如需让 AI / 在线阅读可用，可考虑改用普通 git（但单文件 >100 MB 会超 git 上限），
   或改用 GitHub Releases 托管这两个大文件——**请仓库所有者决定，本系统未擅自改动**。
...
3. **raw.githubusercontent.com 限制**：单文件 <100 MB 的非 LFS PDF 可正常获取；
   `accept-ranges: bytes` 支持，但个别超大 PDF（>50 MB）首次加载较慢，依赖浏览器分段缓存。

4. **CORS**：raw 已发送 `Access-Control-Allow-Origin: *`，跨域读取正常；
   viewer 自带 CSP 为 `connect-src *`，不阻塞。jsdelivr CDN 因本仓库 4.5 GB 超其限制，返回 403，不可用。
